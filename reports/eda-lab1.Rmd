...
>> usa sempre `read_csv` em vez de `read.csv` . e poucas_temp é um nome de variável que pode melhorar um bocado :)

```{r}
poucas_temp = read_csv(here("data/series_from_imdb-all.csv")) %>% 
...
Antes das observações serem feitas, é válido explicar um pouco cada conceito das análises que serão feitas com as séries:
    ...
    - Desvio padrão: mede a dispersão de uma distribuição de dados em relação à sua média, ou seja, valores altos indicam que os valores observados tendem a estar distantes da média, o inverso aplica-se para valores baixos.
    - IQR: avalia a dispersão de dados somente depois de ordená-los em ordem crescente, dessa e dessa forma (faltou dizer :)).
...
>> Sugestões de sintaxes desse plot podem servir para os demais:

```{r}
poucas_temp %>%
    mutate(Séries = series_name) %>%
    ggplot(aes(x = reorder(series_name, UserVotes, FUN = median),
               y = UserVotes,
               color = Séries)) +
    ...
    labs(x = "Nomes das séries",
         ...
         title = "Quantidade de avaliações de cada série pelo IMDB") + 
...
expectativas do público que avaliou a série com notas na faixa de 5 a 7, outros episódios com notas acima dos valores centrais. Já a mais concentrada é American Horror Story, com poucos pontos espalhados fora do bloxplot, sendo assim, isso implica que os usuários avaliaram de uma forma similar os episódios da série.
...
poucas_temp %>%
    ...
    geom_boxplot(width = .5, outlier.colour = NA) + # senão os "outliers" aparecem 2x
    geom_jitter(width = .2, alpha = .7, size = .8) +
...
>> Essa estava ruim de ver por causa da ligação entre episódios em uma mesma temporada...

```{r}
poucas_temp %>% 
    mutate(Séries = series_name) %>%
    ggplot(aes(x = season_ep, y = UserRating, color = Séries)) + 
    geom_line() +
    geom_point(size = .7) +
    facet_grid(Séries ~ season) + 
...
poucas_temp %>%
    mutate(Avaliação = UserRating,
           `Número episódios` = series_ep,
           Série = series_name) %>%
    group_by(Série) %>% 
    mutate(`Posição na série` = `Número episódios` / max(series_ep)) %>% 
    ggplot(aes(
        x = `Posição na série`,
        y = Avaliação,
        color = Série
...
>> Acho que algumas percepções do parágrafo acima aconteciam porque a escala do eixo vertical mudava de figura para figura, certo?
...
>> Acho que tivemos uma falha de comunicação aqui: minha ideia era comparar o 1o e último episódios de cada temporada com os episódios do meio daquela mesma temporada, pra ver se os episódios que começam e terminam a temporada são melhor avaliados que "os do meio".
